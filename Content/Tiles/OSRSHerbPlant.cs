...
{
    // See: https://github.com/tModLoader/tModLoader/blob/1.4.4/ExampleMod/Content/Tiles/ExampleHerb.cs
    ...
    public abstract class OSRSHerbPlant : ModTile
    {
        private const int FrameWidth = 18; // A constant for readability and to kick out those magic numbers

        public abstract int HerbItemType();
        public abstract int HerbSeedType();
            ...
            if (tile.HasTile)
            {
                ...
                else
                {
                    ...
                    if (Main.tileCut[tileType] || TileID.Sets.BreakableWhenPlacing[tileType] || tileType == TileID.WaterDrip || tileType == TileID.LavaDrip || tileType == TileID.HoneyDrip || tileType == TileID.SandDrip)
                    {
                        ...
                        bool harvestableVanillaHerb = Main.tileAlch[tileType] && WorldGen.IsHarvestableHerbWithSeed(tileType, tile.TileFrameX / FrameWidth);
        ...
        public override IEnumerable<Item> GetItemDrops(int i, int j)
        {
            ...
            int herbItemType = HerbItemType();
            int herbItemStack = 1;

            int seedItemType = HerbSeedType();
            int seedItemStack = 0;
            ...
            {
                // Increased yields with Staff of Regrowth when fully grown
                herbItemStack = Main.rand.Next(5, 8);
                seedItemStack = Main.rand.Next(0, 1);
            }
            else if (stage == PlantStage.Grown)
            {
                // Default yields when fully grown
                herbItemStack = Main.rand.Next(3, 6);
            }
